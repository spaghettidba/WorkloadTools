...
namespace WorkloadTools.Listener.ExtendedEvents
{
    public class StreamXEventDataReader : XEventDataReader
    {
        ...
        public override void ReadEvents()
        {
            ...
            */
            var eventstream = new XELiveEventStreamer(ConnectionString, SessionName);
            ...
            Task eventTask = eventstream.ReadEventStream(xevent =>
            {
                ...
                try
                {
                    ...
                    logger.Debug(
                        $"XE={evt.Name}, " +
                        $"Seq={workloadEvent.EventSequence}, " +
                        $"Type={workloadEvent.Type}, " +
                        $"SPID={workloadEvent.SPID}, " +
        ...
        private object TryGetValue(IXEvent evt, FieldType t, string name)
        {
            object result = null;
        ...
        private string TryGetString(IXEvent evt, FieldType t, string name)
        {
        ...
        private int? TryGetInt32(IXEvent evt, FieldType t, string name)
        {
            var tmp = TryGetValue(evt, t, name);
            if (tmp != null && tmp.GetType() != typeof(DBNull))
            {
        ...
        private long? TryGetInt64(IXEvent evt, FieldType t, string name)
        {
            var tmp = TryGetValue(evt, t, name);
            if (tmp != null && tmp.GetType() != typeof(DBNull))
            {
